---
title: "New York Migration Flows"
author: "Jose Zuniga"
...
Brace Expansion is useful for downloading several files via the command line. When downloading annual data where all that exists is year2014to2015.csv and year2015to2016.csv, using `wget` with multiple brace expansions returns errors for the nonexistent files. This is because multiple braces results in a cross (Cartesian) product with every possible combination is returned. Using for loops is an alternative, but can require more coding than using static URLs when a lot of sites are involved. Another issue with Brace Expansions is zero-padding. Any Brace Expansion that passed over “08” returns an errors because numbers with a leading 0 are base 8 octal numbers which return a "too great for base" error.
