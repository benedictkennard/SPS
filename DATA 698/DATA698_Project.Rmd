...
title: |
  ...
  <h3>A Longitudinal Study of the Relationship of Immigration Laws with Changes in Demographics and GDP in the U.S.</h3>
author: "Jose Zuniga"
date: "May 14, 2018"
output:
  html_document:
...
Since the United States passed its first immigration law in 1790,[^1] the U.S. Congress has passed over 200 other immigration laws and amendments an average of once every year.[^2] Legislation has been exclusionary in some cases such as the Chinese Exclusion Act of 1882, and inclusionary on other occasions such as the Chinese Exclusion Repeal Act of 1943. The last immigration law passed was the Javier Vega, Jr. Memorial Act of November 2, 2017. It was a law naming a Border Patrol checkpoint in Texas. According to the White House, the next round of immigration reforms will focus on enforcement, DACA legalization, sponsorship limitations, and Visa restrictions.[^3]  Even though the impact of immigration laws has varied over time, high-impact reforms like those proposed tend to bring intent into question. The intent behind the proposed reforms is currently a controversial topic which cannot be quantified, but it is quite clear that a change in direction is definitely happening. On February 23, 2018 the United States Citizenship and Immigration Services removed the phrase "nation of Immigrants" from its mission statement.
...
```{r warning=F, message=F, fig.align='center', cache=T, echo=F}
# This is for producing tables within the text before the analysis actually begins.
...
# na.omit(data.frame(data698[, 3:10], row.names = data698$Year))
# na.omit(data.frame(data698[, c(3,11:17)], row.names = data698$Year))
```

### U.S. Department of State

Immigrant visas are issued to foreigners permanently relocating to the U.S. and Nonimmigrant visas are issued to foreigners relocating to the U.S. temporarily. Data regarding visas is available both from the U.S. Department of Homeland Security[^24] and the U.S. Department of State.[^25] The data[^26] from the U.S. Department of State shows every Nonimmigrant visa type[^27] by country from 1997 to 2017. This dataset has an extraordinary amount of detail, but only on Nonimmigrant visas issued by consular offices which is only a fraction of the Nonimmigrant visas. 
...
temp <- data698[, 1:2] %>% 
  ...
  spread(Century, Laws) %>% 
  mutate(Decade = ifelse(Decade==0, paste0("0",Decade,"s"), paste0(Decade,"s")))
temp
```
...
```{r warning=F, message=F, fig.align='center', cache=T, echo=F}
# This is for producing tables within the text before the analysis actually begins.
na.omit(data.frame(data698[, 19:26], row.names = data698$Year))
```
...
```{r warning=F, message=F, fig.align='center', cache=T, echo=F}
# This is for producing tables within the text before the analysis actually begins.
na.omit(data.frame(data698[, 28:42], row.names = data698$Year))
```
...
Historic immigration law numbers have an interesting pattern. The period between 1790 and 1931 saw an average of 3.03 pieces legislation per decade. The relative frequency of legislation did increase around the time of Mexican-American War in 1846, but the number of laws passed continued to remain relatively low. Then in 1932, in the midst the Great Depression, the number and frequency immigration laws started to steadily increase. Since 1932 there have been an average of 2.06 immigration laws being passed every year. Looking only at the period since 1997 shows an average of 3.65 immigration laws passed annually. The last 21 years have seen legislation on immigration being enacted at a frequency over 10 times greater than what was witnessed during the first 141 years of Federal legislative action on immigration.
...
ggplot() + ggtitle("Legislation on Immigration (1790-2016)") + xlab("Year") +
  geom_line(aes(y = Laws, x = time(Laws)), stat="identity", color="steelblue") +
  geom_line(aes(y = mov_avg, x = time(mov_avg)), stat="identity", color="red") +
...
The purely numerical data from the decennial Census on race and Hispanic origin has two interesting features. As is usual with most social phenomena, the distributions appear mostly exponential. The two exceptions are the White race which can be described as linear since around 1870 and the Native American race which has a positive parabolic shape with a minimum at the year 1900. For the purposes of aggregate analysis, the free and enslaved Black population that the Census counts separately until 1860 is being counted together. 
...
The decennial numbers on the Native and Foreign-born population also follow what can be described as an exponential pattern. There is a decline in the Foreign-born population from 1940 to 1970, but by 1980 the number returns to its prior levels and continues on its prior trajectory. 

```{r warning=F, message=F, fig.align='center', cache=T}
# plot(na.approx(NAFB), type="l", main="Native and Foreign-born (Totals)")
...
The decennial numbers on country of origin for Foreign-born are a little more interesting. Persons from Africa, Asia, Oceania, and Latin America arrive at an exponential rate. Arrivals from Europe and North America however, show exponential growth until around 1930, and then steady declining thereafter. 
...
The numeric data on Immigrant Visas since 1820 is best described as chaotic. It has wild swings up and down starting with an upward trend from 1820 to about 1910, followed by a drastic decline until about 1945, and then another trend upward with a similar trajectory until 2016. The graph also shows a huge spike around 1990. 

```{r warning=F, message=F, fig.align='center', cache=T}
IV_All <- na.omit(Visas[, 2])
numb_df <- data.frame(IV_All, Year=time(IV_All))
ggplot() + ggtitle("Immigrant Visas (1820-2016)") + xlab("Year") + ylab("Amount") +
  geom_line(aes(y = IV_All, x = time(IV_All)), stat="identity", color="steelblue") +
  theme(plot.title = element_text(hjust = 0.5))
```
...
```{r warning=F, message=F, fig.align='center', cache=T}
Enforce <- window(data698[, 23:25], start = 1925)
```

Enforcement data has some interesting patterns. Removals remained at a constant rate until 1996 when they increased substantially. Apprehensions and Returns follow almost identical patterns. There was a large increase in enforcement in the ten-year period between 1945 and 1955 with its peak being reached in 1954. The increase started again in 1965 and has remained elevated ever since with peaks in 1986, 1996, and 2000. Around the time Removals started steadily increasing, Apprehensions and Returns started steadily decreasing. The three enforcement actions combined follow the pattern of the latter two enforcement actions which make up the bulk of the actions. 

```{r warning=F, message=F, fig.align='center', cache=T}
numb_df <- data.frame(Enforce, Year=time(Enforce))
plot_df <- gather(numb_df, "Mehod", "Actions", 1:(ncol(numb_df)-1))
ggplot(plot_df, aes(x=Year, y=Actions)) + geom_line() + 
  facet_wrap(~ Mehod, scales="free_y") + theme_bw() +
  ggtitle("Enforcement by Mehod (1981-2016)") + 
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_y_continuous(labels = scales::comma)
ggplot(plot_df, aes(x=Year, y=Actions, fill= Mehod)) + geom_area() +
  ggtitle("Enforcement by Mehod (1981-2016)") + xlab("Year") +
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_y_continuous(labels = scales::comma) + 
  scale_fill_grey(start = 0.2, end = 0.8)
```
...
```{r warning=F, message=F, fig.align='center', cache=T}
Economy <- window(data698[, 27:42], start = 1987)
```

GDP by industry over the past 30 years has increased nearly uniformly across industries. The exceptions are Agriculture Mining, Utilities and Construction which appear to be more susceptible to business cycles. 

```{r warning=F, message=F, fig.align='center', cache=T}
numb_df <- data.frame(Economy, Year=time(Economy))
...
ggplot(plot_df, aes(x=Year, y=Income, fill= Sector)) + geom_area() +
  ggtitle("GDP by Sector (1987-2016)") + xlab("Year") +
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_y_continuous(labels = scales::comma) + 
  scale_fill_grey(start = 0.2, end = 0.8)
```

When examining the industries in aggregate as a proportion of the economy, their shares appear to remain relatively constant over the last 30 years. The Manufacturing, FIRE (Finance, Insurance, Real Estate), Service, and Education sectors make up the bulk of GDP; in that order. Over time there has been a slight decrease in the proportion of the economy captured by the Manufacturing sector and slight increases in the FIRE and Service sectors
...
```{r warning=F, message=F, fig.align='center', cache=T}
Legislation <- window(cbind(Law_New=Laws, Law_1yr=stats::lag(Laws, k=-1),
  Law_2yr=stats::lag(Laws, k=-2), Law_3yr=stats::lag(Laws, k=-3)), end=2016)
```

### Social Factors

First the Kendall correlation between new, one-year old, two-year old, and three-year old immigration legislation and social factors is examined. When it comes to Race and Hispanic Origin, legislation has correlations between $\tau=0.51$ and $\tau=0.56$ for Hispanic origin and nearly every race. The exception is Native Americans which has a correlation that hoovers around $\tau=0.26$. All of these correlations are significant at $\alpha=0.05$.

```{r warning=F, message=F, fig.align='center', cache=T}
corr.test(na.approx(RAHO), Legislation, use="complete.obs", method="kendall")$r
corr.test(na.approx(RAHO), Legislation, use="complete.obs", method="kendall")$p < 0.05
```


For the Native and Foreign-born population, the correlations with immigration law are also moderate. Native have a correlation that hoovers around $\tau=0.52$. Foreign-born have a correlation that hoovers around $\tau=0.44$. All of these correlations are significant at $\alpha=0.05$.

```{r warning=F, message=F, fig.align='center', cache=T}
corr.test(na.approx(NAFB), Legislation, use="complete.obs", method="kendall")$r
corr.test(na.approx(NAFB), Legislation, use="complete.obs", method="kendall")$p < 0.05
```

The correlations of Europe and North America with immigration law are between $\tau=0.19$ and $\tau=0.26$ with the lower correlations figures being from Europe. All of these correlations are significant at $\alpha=0.05$.

```{r warning=F, message=F, fig.align='center', cache=T}
corr.test(na.approx(FBCO), Legislation, use="complete.obs", method="kendall")$r
corr.test(na.approx(FBCO), Legislation, use="complete.obs", method="kendall")$p < 0.05
```

### Economic Factors

The Kendall correlation between new, one-year old, two-year old, and three-year old immigration legislation and economic factors is examined next. The largest correlations with immigration legislation and visas can be found after two years. Although these correlations are larger, they are not very high. The correlations are between $\tau=0.21$ and $\tau=0.31$. Only one of these correlations is significant at $\alpha=0.05$. The significant correlation found is between two-year lagged immigration laws and all Immigrant visas aggregated without regard to type. This could imply a relationship between temporary visas and legislations after two years, but it could also be a statistical anomaly. Within the context of statistical tests, at least one in twenty false positives are expected are a 95% confidence level.

```{r warning=F, message=F, fig.align='center', cache=T}
corr.test(Visas, Legislation, use="complete.obs", method="kendall")$r
corr.test(Visas, Legislation, use="complete.obs", method="kendall")$p < 0.05
```

Enforcement actions have low correlations with immigration law that are fairly constant across lags. They range between $\tau=0.19$ and $\tau=0.24$. None of these correlations are significant at $\alpha=0.05$.

```{r warning=F, message=F, fig.align='center', cache=T}
corr.test(Enforce, Legislation, use="complete.obs", method="kendall")$r
corr.test(Enforce, Legislation, use="complete.obs", method="kendall")$p < 0.05
```

The largest correlations with immigration legislation and GDP by Industry can be found after three years. Just like before, although these correlations are larger, they are not very high. Excluding the highest and lowest correlations, these correlations range between $\tau=0.29$ and $\tau=0.33$. The highest correlation is with Utilities at $\tau=0.39$ and the lowest is with Agriculture at $\tau=0.25$. None of these correlations are significant at $\alpha=0.05$.

```{r warning=F, message=F, fig.align='center', cache=T}
corr.test(Economy, Legislation, use="complete.obs", method="kendall")$r
corr.test(Economy, Legislation, use="complete.obs", method="kendall")$p < 0.05
```

## Economic Correlations

As previously mentioned, Kendall correlations between legislation and the economy are low and statistically insignificant. This is not true for many other factors. Correlations between Race and Hispanic Origin and every industry in the Economy are high and statistically significant at $\alpha=0.05$ for every single race and Hispanics. 

```{r warning=F, message=F, fig.align='center', cache=T}
round(corr.test(Economy, na.approx(RAHO), use="complete.obs", method="kendall")$r, 5)
corr.test(Economy, na.approx(RAHO), use="complete.obs", method="kendall")$p < 0.05
```

The same is true for the Native and Foreign-born population and every industry. Correlations are high and statistically significant at $\alpha=0.05$. 

```{r warning=F, message=F, fig.align='center', cache=T}
corr.test(Economy, na.approx(NAFB), use="complete.obs", method="kendall")$r
corr.test(Economy, na.approx(NAFB), use="complete.obs", method="kendall")$p < 0.05
```

Country of origin does not show the same level of equality however. At an $\alpha=0.05$ Asia, Africa, Oceania, and Latin America have high and statistically significant correlations with every industry, but Europe and North America have statistically insignificant correlations with every industry. 

```{r warning=F, message=F, fig.align='center', cache=T}
corr.test(Economy, na.approx(FBCO), use="complete.obs", method="kendall")$r
corr.test(Economy, na.approx(FBCO), use="complete.obs", method="kendall")$p < 0.05
```

There is also a dichotomy between Immigrant and Nonimmigrant visas. At an $\alpha=0.05$, Nonimmigrant visas have statistically significant correlations with every industry, but Immigrant visas have statistically insignificant correlations with every industry except for Utilities. Much like the outlier noted in the correlations between Immigrant Visas and Legislation, this could imply either a relationship or be a statistical anomaly because within the context of statistical tests, at least one in twenty false positives are expected are a 95% confidence level.

```{r warning=F, message=F, fig.align='center', cache=T}
corr.test(Economy, Visas, use="complete.obs", method="kendall")$r
corr.test(Economy, Visas, use="complete.obs", method="kendall")$p < 0.05
```

Enforcement actions also have mixed results. Apprehensions and Returns have statistically insignificant correlations with every industry, but Removals have statistically significant correlations with every industry at $\alpha=0.05$. As stated earlier, Removals are compulsory actions based on a court order and Returns represent immigrants stopped (without a court order) from entering at the borders.

```{r warning=F, message=F, fig.align='center', cache=T}
corr.test(Economy, Enforce, use="complete.obs", method="kendall")$r
corr.test(Economy, Enforce, use="complete.obs", method="kendall")$p < 0.05
```

# Discussion

The correlations between legislation on immigration and the social factors examined are greater than the correlations between immigration legislation and the economic factors examined. The finding that the correlations between immigration laws and Native Americans is small makes sense intuitively. Native Americans are not generally impacted by legislation on immigration. The finding that lagging has little effect on correlations with the social factors examined is indicative of a temporally close relationship between the two. Although practically no significant correlations were found between immigration laws and economic factors, a two-year lagged temporal relationship between Immigrant visas and immigration laws was found to be significant. At a 95% confidence level, this could be a statistical anomaly. It seems reasonable however that Immigrant visas which foreigners obtain to permanently relocate to the United States are impacted by legislation that was enacted two years prior. Planning a move from one country to another is quite an undertaking that may very well be planned two years in advance. Overall, from the factors examined, it was found that social and not economic factors have statistically significant correlations with legislation on immigration.

The correlations between social factors and various industries in the economy contradicts several stereotypes. The finding that every race, Hispanic, Natives, and Foreign-born have a strong and statistically significant positive correlation with every industry in the economy is refreshing. It was a little interesting to see that the European and North American subsections of the Foreign-born did not have statistically significant correlations with any industry. This is likely related to European and North American arrivals having an unnatural distribution that was inflated until race stopped being a factor in admissions around 1950. It was also interesting to see how the temporary Nonimmigrant visas had a statistically significant correlation with the economy while the permanent Immigrant visas had statistically insignificant correlations. The exception to this finding was Immigrant Visas which has a statistically significant relationship with Utilities. This, coupled with how Immigrant visas had the only significant correlation with immigration laws, shows that Immigrant visas may not just be a statistical anomaly at the 95% confidence level. This is indicative of Immigrant visas being tied into the U.S. economy. It is easy to see how permanent relocations impact Utilities. What is strange is how Immigrant visas do not share a statistically significant relationship with other parts of the economy. Finally, it was also strange to see that court ordered Removals, but not other enforcement actions, had a statistically significant positive correlation with industry. It makes sense however when looking at the definitions. Apprehensions and Returns are swift actions that involve a limited number of parties. Court ordered removals are prolonged processes that involve several institutions such as government agencies, private prison systems, and law firms.

# Conclusion

Historically, legislation on immigration has had a statistically insignificant correlation with the economy. The statistically significant correlations that immigration laws have had are with demographics. Based on history, the economy will improve regardless of the demographic makeup of immigrants. Politicians claiming things will improve if a change is made, knowing that improvement is likely regardless of the change, are exercising political savvy in advancing their interests. Much can be said about the ethics behind such self-serving behavior, [^51] but only a reference to further reading on the topic will be provided in this analysis. Although intent cannot be inferred, this research does show that it is more likely that opponents of the current presidential administration are correct in stating that the proposed reforms will reduce the diversity of Immigrants rather than benefit the economy. The results support the theory in showing a lack of meaningful correlation with changes in the legal flow of immigrants and economic growth over America’s history.
