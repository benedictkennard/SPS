...
```{r warning=F, message=F, fig.align='center', cache=TRUE, echo=F}
...
data698 <- ts(data698[, -1], frequency=1, start=min(data698[, 1]), end=max(data698[, 1]))
Census <- data698[, 2:17]
na.omit(Census)
```
...
```{r warning=F, message=F, fig.align='center', cache=TRUE, echo=F}
# This is for producing tables within the text before the analysis actually begins.
USCIS <- data698[, c(18:21, 23:25)] # Why is 22 missing?
na.omit(USCIS)
```
...
lines(mov_avg, col=2) # Add legend for lines
...
```{r warning=F, message=F, fig.align='center', cache=TRUE}
plot(Visas[, 2], ylab="Imm_All", main="Immigrant Visas (Total)")
```
...
The correlations or Europe and North America with immigration law are between $\tau=0.19$ and $\tau=0.26$ with the lower correlations figures being from Europe. All of these correlations are significant at $\alpha=0.05$.
...
The Kendall correlation between new, one-year old, two-year old, and three-year old immigration legislation and economic factors is examined next. The largest correlations with immigration legislation and visas can be found after two years. Although these correlations are larger, they are not very high. The correlations are between $\tau=0.21$ and $\tau=0.31$. None of these correlations are significant at $\alpha=0.05$.
...
There is also a dichotomy between Immigrant and Nonimmigrant visas. At an $\alpha=0.05$, Nonimmigrant visas have statistically significant correlations with every industry, but Immigrant visas have statistically insignificant correlations with every industry. 
...
Historically, legislation on immigration has had a statistically insignificant correlation with the economy. The statistically significant correlations that immigration laws have had are with demographics. Based on history, the economy will improve regardless of the demographic makeup of immigrants. Politicians claiming things will improve if a change is made, knowing that improvement is likely regardless of the change, are exercising political savvy in advancing their interests. Much can be said about the ethics behind such self-serving behavior, [^51] but only a reference to further reading on the topic will be provided in this analysis. It can be asserted however is that although intent cannot be inferred, this research shows that it is more likely that opponents of the current presidential administration are correct in stating that the proposed reforms will reduce the diversity of Immigrants rather than benefit the economy. The results support the theory in showing a lack of meaningful correlation with changes in the legal flow of immigrants and economic growth over America’s history.
