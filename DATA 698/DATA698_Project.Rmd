...
```
https://stackoverflow.com/questions/29697009/correlation-matrix-plot-with-ggplot2?rq=1
chart.Correlation(my_data, histogram = F, pch = 19)
https://www.rdocumentation.org/packages/GGally/versions/1.3.2/topics/ggpairs
https://www.rdocumentation.org/packages/PerformanceAnalytics/versions/1.5.2/topics/chart.Correlation
https://www.rdocumentation.org/packages/graphics/versions/3.4.3/topics/pairs
https://stackoverflow.com/questions/21810675/ggallyggpairs-plot-without-gridlines-when-plotting-correlation-coefficient
--> 
...
ggplot(plot_df, aes(x=Year, y=Population, fill= Demographic)) + geom_area() +
  ...
  scale_y_continuous(labels = scales::comma) + 
  scale_fill_grey(start = 0.2, end = 0.8)
na.omit(data.frame(round(Proportions, 6)), row.names=as.numeric(time(Proportions)))
```
