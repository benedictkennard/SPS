...
Pearson ($r$) correlation measures the strength of relationships between linear variables. This analysis involves nonlinear variables therefore the metric used is Kendall[^49] correlation ($\tau$) which is a non-parametric measure of the strength of relationships between nonlinear variables. These correlations are then tested for statistical significance. That is, is the existence of the correlations significantly different from zero given approximately normal variation (and covariance) in the data.

$$r = \frac{\sum{(x-m_x)(y-m_y)}}{\sqrt{\sum{(x-m_x)^2}\sum{(y-m_y)^2}}} `r ifelse((knitr::opts_knit$get('rmarkdown.pandoc.to') == 'docx'), '\\quad\\textrm', '\\tag')`{Pearson}$$$$\tau = \frac{n_c - n_d}{\frac{1}{2}n(n-1)} `r ifelse((knitr::opts_knit$get('rmarkdown.pandoc.to') == 'docx'), '\\quad\\textrm', '\\tag')`{Kendall}$$
...
```
```{r  cache=T, echo = F, eval=!knitr::is_html_output()}
ggplot(plot_df, aes(x=Year, y=Approved, fill= Type)) +
  ggtitle("Immigrant and Nonimmigrant Visas by Type (1981-2016)") +
  geom_area(alpha = 0.75) + xlab("Year") +
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_y_continuous(labels = scales::comma) + 
...
[^56]: https://github.com/jzuniga123/SPS/blob/master/DATA%20698/Immigration_Legislation.pdf