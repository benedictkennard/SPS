...
The dataset compiled from all these Census data is quite impressive, but it is not yet ready for use. Several nuances must be addressed.  Although analyzing the cause of these discrepancies is not a subject of this paper, it is worth noting that -while not surprising given the history of the hegemony in the United States-the discrepancies are quite unsettling. Notes from the metadata explain that:
  ...
  5.  The Census data obtained is decennial from 1790 to 2010 and then annual until 2016. All other non-Census data is annual and complete from 1987 until 2016. 

To address the discrepancy caused by (1), the number of slaves disembarking in the United States[^21] is used as a proxy for Foreign-born Africans entering the United States from 1790 to 1880. To address (2), the Native-American population figures are backfilled using the percent change[^22] of the Native-American population starting from the earliest period without discrepancies which is 1890. Native born and total population figures are also adjusted to reflect the revised Native-American population figures. To address (3), the value was removed since it was not representative of all Hispanics. Then, to complete the dataset, imputation is used. The missing data outlined in (4) for the Foreign-born in 1940 and 1950 is filled in using linear interpolation between 1930 and 1960. To conduct a proper analysis of the past 30 years, the missing data outlined in (5) is also filled in using linear interpolation between periods that there are data points present.
...
kable(Arima_Orders, longtable = T, booktabs = T, row.names=T, linesep = "",
  caption = "Time Series ARIMA Modeling") %>%
  kable_styling(latex_options = c("striped", "hold_position", "repeat_header"))
```
...
kable(Causality, longtable = T, booktabs = T, row.names=T, linesep = "",
  caption = "Legislation-Demographic Causality Test") %>%
...
```{r cache=F, echo=knitr::is_html_output(), eval=knitr::is_html_output()}
par(mfrow=c(2,2))
plot(LawsCause$Hispanic, type="l")
plot(LawsCause$Native_Born, type="l")
plot(LawsCause$Africa, type="l")
plot(LawsCause$Latin_Am, type="l")
```
```{r cache=F, echo=knitr::is_html_output(), eval=T, results = 'asis'}
...
`r if(!knitr::is_html_output()){tune01$modelInfo$label}`

```{r  cache=T, echo = F, eval=!knitr::is_html_output()}
plot(tune01)
```

`r if(!knitr::is_html_output()){tune02$modelInfo$label}`
...
`r if(!knitr::is_html_output()){tune03$modelInfo$label}`

```{r  cache=T, echo = F, eval=!knitr::is_html_output()}
plot(tune03)
```

`r if(!knitr::is_html_output()){tune04$modelInfo$label}`
...
`r if(!knitr::is_html_output()){tune05$modelInfo$label}`

```{r  cache=T, echo = F, eval=!knitr::is_html_output()}
plot(tune05)
```

`r if(!knitr::is_html_output()){tune06$modelInfo$label}`
...
`r if(!knitr::is_html_output()){tune07$modelInfo$label}`

```{r  cache=T, echo = F, eval=!knitr::is_html_output()}
plot(tune07)
```

`r if(!knitr::is_html_output()){tune08$modelInfo$label}`
...
`r if(!knitr::is_html_output()){tune09$modelInfo$label}`

```{r  cache=T, echo = F, eval=!knitr::is_html_output()}
plot(tune09)
```

`r if(!knitr::is_html_output()){tune10$modelInfo$label}`
...
`r if(!knitr::is_html_output()){tune11$modelInfo$label}`

```{r  cache=T, echo = F, eval=!knitr::is_html_output()}
plot(tune11)
```

`r if(!knitr::is_html_output()){tune12$modelInfo$label}`
