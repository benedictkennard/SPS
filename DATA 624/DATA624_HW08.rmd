...
Conditional variable importance scores are supposed to reflect “the true impact of each predictor variable more reliably than” the standard variable importance score calculation method. This however, does not seem to be the case here.  For the Conditional Inference Trees (CIT), the conditional importance (CI) scores follow the same ranking pattern as the standard importance (SI) scores for the informative variables of the simulated and highly correlated data. For the uninformative predictors the pattern is nearly with same as well. Inclusively, the decrease in importance of the informative variables when a highly correlated variable is included is greater when using the conditional importance (CI) scores.
...
After fitting tree-based CART, RF, and SGB models to the ` permeability` data which were previously found to be best suited for fitting with an ENT or SVM model, it appears that the SGB model provides the most suitable fit for the `permeability` data. The SGB model outperforms the other tree-based models in every resampling performance metric. The RF model outperforms the other tree-based models on the test set, but these test set metrics are less important to predictive ability than the resampling performance metrics.
...
After fitting tree-based CART, RF, and SGB models to the ` permeability` data which were previously found to be best suited for fitting with a linear ENT or nonlinear SVM model, it appears that the SGB model provides the most suitable fit for the `permeability` data. The ENET model outperforms the other tree-based models in every resampling performance metric. The RF model outperforms the ENET model on the test set, but these test set metrics are less important to predictive ability than the resampling performance metrics.
...
Using the resampling performance metrics as a guide in choosing the recommendation of a replacement for the permeability laboratory experiment, it would probably be best to opt for the ENT model. This model performs better than the SVM and tree-based models. If, however, a more interpretable model is necessary for business users, the CART model would be a good choice. Its resampling performance is relatively close to that of the ENET model and it is more interpretable to business users.
...
The CUBE model which was found to be the optimal tree-based regression model, the KNN model which [was found](https://rpubs.com/josezuniga/376348) to be the optimal nonlinear regression model, and the PLS model which [was found](https://rpubs.com/josezuniga/366918) to be the optimal linear regression model all agree that the most important predictor is the `ManufacturingProcess32` process variable. All models also agree that `BiologicalProcess32` is a very important biological process, but the CUBE model ranks it equal in importance to the `BiologicalMaterial03` biological process. The PLS and KNN models clear distinguish `BiologicalProcess32` as the most important biological process and `BiologicalMaterial03` as the second most important biological process. Another interesting difference in the CUBE model is that only five predictors are listed as important, one of which (`ManufacturingProcess25`) is not even in the top ten of the PLS or the KNN models. 
...
The optimal single tree model based on RMSE, $R^2$, and MAE resampling performance metrics is the CIT model. This decision tree produced for the CIT model shows a single split for `ManufacturingProcess32` at the 0.111 value. Values lower than 0.111 are associated with lower yields as represented by the boxplots. Values over 0.111 are associated with higher yields. This view of the tree does help in understanding the relationship between `ManufacturingProcess32` and the yield, but very little due to the single split. The CART model tree is plotted for comparison. This tree has several branches and details the same information about `ManufacturingProcess32`, albeit at a different cutoff value of 0.2, but also provides a much greater understanding of the relationship between the biological and process predictors and the yield due to the additional branches.
