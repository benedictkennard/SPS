...
Adding the predictor `V11` that is highly correlated with the most informative predictor `V1` does change the variable importance score of `V1` as well as the importance scores of all the other informative and uninformative variables. The importance of this highly correlated `V11` predictor even outranks two of the five informative variables. This highlights a weakness in tree-based models. When variables are highly correlated, the models have a tendency to select more predictors than are actually needed with the choice between highly correlated predictors being nearly random.
...
The `cforest()` implementation of the random forest (and bagging) algorithm differs from `randomForest()` with respect to the base learners used and the aggregation scheme applied. Conditional inference trees are fitted to each of the `ntree` (default 500 or defined via `cforest_control()` bootstrap samples of the learning sample. The only hyperparameter set in `cforest_control()` is `mtry` which dictates the number of randomly preselected variables. This hyperparameter is set to $P-1$, where $P$ is the number of predictors instead of the `cforest()` default of five (in `randomForest()` the default varies with the number of input variables). After a  `cforest()` model is fit, there are two functions that can calculate the importance of the variables. The `varimp()` function with a lowercase **i** from the `party` package calculates the standard and conditional variable importance for `cforest()` following the permutation principle of the "mean decrease in accuracy" importance in `randomForest()`. The `varImp()` function with an uppercase **I** from the `caret` package is a generic method for calculating variable importance for objects produced by `train()` and other specific methods such as `cforest()`. For the sake of consistency, the `varImp()` function with an uppercase **I** is used. Again, the variable importance calculations for all the models are then output is a data frame for comparison. To create the data frame models without the `V11` predictor need to be padded with a `V11` predictor that has a variable importance value of `NA`.
...
data.frame(rbind(varimp1, padding1), varimp2, 
  rbind(varimp3, padding2), varimp4,
  rbind(varimp5, padding3), varimp6)
```
...
The random forest implementations randomForest and cforest are more flexible and reliable for computing bootstrap-aggregated trees than the `bagging()` function and should be used instead.
