...
duplicated$V11 <- duplicated$V1 + rnorm(200) * .1
...
Standard and conditional variable importance for `cforest`, following the permutation principle of the "mean decrease in accuracy"" importance in `randomForest`. `conditional` is a logical determining whether unconditional or conditional computation of the importance is performed.
...
colnames(varimp1) <- "Simulated_CIT_UI" 
colnames(varimp2) <- "Correlated_CIT_UI"
colnames(varimp3) <- "Simulated_CIT_CI" 
colnames(varimp4) <- "Correlated_CIT_CI"
padding1 <- data.frame(Simulated_CIT_UI=NA, row.names="V11")
padding2 <- data.frame(Simulated_CIT_CI=NA, row.names="V11")
data.frame(rbind(varimp1, padding1), varimp2, rbind(varimp3, padding2), varimp4)
```
...
```{r warning=F, message=F, fig.align='center', cache=T}
set.seed(624)
fit3 <- bagging(y ~ ., simulated, nbag = 50)
fit4 <- bagging(y ~ ., duplicated, nbag = 50)
```
```{r warning=F, message=F, fig.align='center', cache=T}
set.seed(624)
fit5 <- cubist(x=simulated[,-11], y=simulated[,11], committees = 100)
fit6 <- cubist(x=duplicated[,-11], y=duplicated[,11], committees = 100)
```
```{r warning=F, message=F, fig.align='center', cache=T}
varimp1 <- varImp(fit3)
varimp2 <- varImp(fit4)
varimp3 <- varImp(fit5)
varimp4 <- varImp(fit6)
colnames(varimp1) <- "Simulated_BAG" 
colnames(varimp2) <- "Correlated_BAG"
colnames(varimp3) <- "Simulated_RBC"
colnames(varimp4) <- "Correlated_RBC"
padding1 <- data.frame(Simulated_BAG=NA, row.names="V11")
padding2 <- data.frame(Simulated_RBC=NA, row.names="V11")
data.frame(rbind(varimp1, padding1), varimp2, rbind(varimp3, padding2), varimp4)
```
...
```{r warning=F, message=F, fig.align='center', cache=T}
models <- c("CART","RF","RBC_1.0","RBC_1.n","RBC_C.0","RBC_C.n")
...
data.frame(rbind(metrics(tune1), metrics(tune2), metrics(tune3), 
  ...
  row.names = c("PLS","CART","BAG","RF","SGB","RBC"))
fits <- list(PLS=tune1, CART=tune2, BAG=tune3, RF=tune4, SGB=tune5, RBC=tune6)
...
To tune an CART tree over the complexity parameter, the method option in the train function should be set to `rpart`. To tune over maximum depth, the method option should be set to `rpart2`.
...
The number of randomly selected predictors to choose from at each split is set by `mtry`. since random forests is computationally intensive, tune `mtry` with around five values that are somewhat evenly spaced across the range from 2 to $P$, where $P$ is the number of predictors.
...
Specify the minimum number of samples needed to further splits with $M$.
...
data.frame(model="RBC", tune9$bestTune, RMSE=min(tune9$results$RMSE), row.names="")
plot(tune9)
```
...
data.frame(rbind(metrics(tune1), metrics(tune2),
  ...
  row.names = c("PLS","KNN","CART","CIT","M5","BAG","RF","SGB","RBC"))
```
