...
The `summary()` function provides descriptive statistics on quantitative and qualitative variables. The `xts::periodocity()` function describes the periods of the dates in the time series. The `complete.cases()` function returns observations in the datasets that have no missing values.
...
The Box-Cox transformation parameters suggested are around $\lambda=0.5$. This more interpretable (rounded) value is suggestive of a ${ 1 }/{ \sqrt { y_t } }$ transformation. This Box-Cox transformation stabilizes the variance and makes the series look relatively homoskedastic with equal variance. 
...
The `tsclean()` function removed the missing value but did not remove the outlier. The outlier was manually replaced with the mean of the series. The `frequency` parameter was selected in accordance with the documentation accompanying the `ts()` function:
...
The 6-month and 12-month (year) moving average smoother line shows that the data only a slight apparent trend.
...
The Box-Cox transformation parameter suggested is about $\lambda=-0.2$. This Box-Cox transformation stabilizes the variance and makes the series look relatively homoskedastic with equal variance. 
...
The residuals of the models appear to display the characteristics of White Noise in the ACF and PACF plots.
...
The null hypothesis of independence is not rejected. The Box-Ljung shows that the autocorrelations of the residuals from the model is not significantly different from zero. The residuals of the models display the characteristics of White Noise. The model passes the required checks and is therefore suitable for forecasting.
...
These accuracy for the predications is fair. The large RMSE and other metric are representative of the large values found in the data. 
...
The data for Pipe 1 which have a period of very ten minutes were converted to hourly with the `mean()` function applied to the aggregated hourly periods.
...
The `merge()` function takes two arguments. Therefore, `Pipe1` and `Pipe2` are first merged using an inner join, then there sum of the Pipes was merged to resultant multi-series.
...
The hourly aggregate of Pipe 1 is being used in this analysis. The `frequency` parameter was selected in accordance with the documentation accompanying the `ts()` function:
...
The 3.5-day (1/2 week) and 15-day (1/2 month) moving average smoother line shows that the data for the ATMs have no apparent trend.
...
The Box-Cox transformations suggested are around $\lambda=1$. This more interpretable (rounded) value is suggestive of no transformation. It will still be applied however since Box-Cox transformations stabilizes the variance and make the series look relatively homoskedastic with equal variance. 
...
The predictions appear to produce a useful forecasts that do not reflect patterns in the original data. The predictions intervals capture the actual data but the point estimates revert to the mean. This is likely due to the original data being Gaussian White Noise.
...
The closer data are to being White Noise, the less accurate the predictions. White Noise is not predicable. The accuracy for these predications is without meaning. 
