...
Do NYC Police Precincts and NYC Hospitals report different mortality figures? This question will be examined in general terms (whether NYC Police Precincts and NYC Hospitals report the same total mortality figures in their statistics) and much more specific terms (whether mortality statistics are skewed toward hospitals near police precincts).
...
The data reported by the NYC PD and NYC DOHMH is conflicting and flawed. The NYC PD data shows `r nrow(Precincts_1)` years of data with an average of `r round(mean(unlist(Precincts_1[2])), 0)` while the NYC DOHMH data shows `r nrow(Hospital_1)` years of data with an average of `r round(mean(unlist(Hospital_1[2])), 0)`. When looking at the years where both have data available, the average difference is `r round(mean(unlist(Compare_2[3] - Compare_2[2])), 0)` which is very strange. The NYC DOHMH data is inflated by a constant factor of about `r round(mean(unlist(Compare_2[3] / Compare_2[2])), 2)`.
...
Given latitudes $\lambda_{ 1 }, \lambda_{ 2 }$ and longitudes $\varphi _{ 1 }, \varphi _{ 2 }$ in radians, and the Earth's mean radius in miles of $R=3959$, the distance between two points in miles can be calculated with the below formula.
...
The police precincts and hospitals plotted on a map show relatively even spacing between police precincts and hospitals. Zooming in further also shows how close everything is to each other. The scatter plot makes this even more obvious with the vast majority of distances clustered in the area representing less than two miles. It is also worth noting the difference between mortality totals and mortality frequency relative to population.
